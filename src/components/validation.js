...
const showInputError = (formElement, inputElement, errorMessage, config) => {
  const errorElement = formElement.querySelector(`#${inputElement.id}-error`);
  inputElement.classList.add(config.inputErrorClass); // Добавляем класс ошибки для поля
  errorElement.textContent = errorMessage; // Устанавливаем текст ошибки
  errorElement.classList.add(config.errorClass); // Добавляем класс ошибки для текста
};

// Функция для скрытия ошибки валидации
const hideInputError = (formElement, inputElement, config) => {
  const errorElement = formElement.querySelector(`#${inputElement.id}-error`);
  inputElement.classList.remove(config.inputErrorClass); // Убираем класс ошибки для поля
  errorElement.classList.remove(config.errorClass); // Убираем класс ошибки для текста
...
const isValidImageUrl = (url) => {
  return new Promise((resolve) => {
    ...
    img.onload = () => resolve(true); // URL ведет на изображение
    img.onerror = () => resolve(false); // URL не ведет на изображение
  });
};
...
const toggleButtonState = (inputList, buttonElement, config) => {
  if (hasInvalidInput(inputList)) {
    ...
    buttonElement.classList.remove(config.inactiveButtonClass); // Делаем кнопку активной
    buttonElement.disabled = false;
  }
};
...
export const enableValidation = (config) => {
  ...
  formList.forEach((formElement) => {
    ...
    setEventListeners(formElement, config); // Устанавливаем обработчики событий
  });
};
